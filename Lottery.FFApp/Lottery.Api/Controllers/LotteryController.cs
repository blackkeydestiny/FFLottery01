...
namespace Lottery.Api.Controllers
{
    ...
    public class LotteryController : BaseController
    {
        ...
        [CrossSite]
        [Description("获取下注数据")]
        public Result<string> GetBetDataListByLotteryIdAndUserId(string lotteryId, string userId)
        {
            try
            {
                var result = this.LotteryService.GetBetDataListByLotteryIdAndUserId(lotteryId, userId);
                return GetSuccessResult(result);
            }
            catch (Exception ex)
            {
